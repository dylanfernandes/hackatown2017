...
//#define IMAGE_FILE_TO_READ  MIL_TEXT("c:\\input\\licenseplate2.bmp")
...
/*****************************************************************************/
/* Main. */
//eg. "C:\\input\\licenseplate.bmp" "c:\\input\\toto.txt" true
int main(int argc, char *argv[])
   ...
   MIL_TEXT_CHAR IMAGE_FILE_TO_READ[256];
   MIL_TEXT_CHAR OUTPUTFILE[256];
   MOs_strcpy(IMAGE_FILE_TO_READ, argv[1]);
   MOs_strcpy(OUTPUTFILE, argv[2]);
   ...
   */
   ...
   MIL_INT ImgSizeX = MbufDiskInquire(IMAGE_FILE_TO_READ, M_SIZE_X, M_NULL);
   MIL_INT ImgSizeY = MbufDiskInquire(IMAGE_FILE_TO_READ, M_SIZE_Y, M_NULL);
   MIL_ID MilImage2 = MbufAlloc2d(MilSystem, ImgSizeX, ImgSizeY, 8+M_UNSIGNED, M_IMAGE+M_PROC+M_DISP, M_NULL);
	   
	   MbufLoad(IMAGE_FILE_TO_READ, MilImage2);
   ...
   MstrRead(MilStrContext, MilImage2, MilStrResult);
   ...
   for(i=0; i<256; i++)
   {
	   if((StringResult[i] >= 48) && (StringResult[i] << 122))
		   continue;
	   else
		   break;
   }
   ...
   //write output file
   FILE * licensePlateOutput = fopen(OUTPUTFILE, "w");
   if(licensePlateOutput)
   {
	   fwrite(StringResult, sizeof(MIL_TEXT_CHAR), i, licensePlateOutput);
	   fclose(licensePlateOutput);
   }
